...
type FetcherService interface {
	GetClassifyInfo(classify string) []*novel.BookInfo                                      // 分类书籍查找
	GetItemList(keyword string) []*novel.BookInfo                                           // 关键字查找
	GetItem(url string, key string) novel.BookInfo                                          // 书籍详情
	GetChapterList(url string, key string) []novel.Chapter                                  // 章节列表
	GetContent(detailURL string, chapterURL string, key string) (content novel.BookContent) // 获得内容
}
...
// 分类书籍查找
func (s *fetcherService) GetClassifyInfo(classify string) (itemList []*novel.BookInfo) {
...
// 关键字搜索
func (s *fetcherService) GetItemList(keyword string) (itemList []*novel.BookInfo) {
	bookSources := sourceService.GetAllSource()
...
func (s *fetcherService) GetItem(url string, key string) (info novel.BookInfo) {
...
func (s *fetcherService) GetChapterList(url string, key string) (chapterList []novel.Chapter) {
	source, ok := sourceService.GetSourceByKey(key)
	if !ok {
		return
	}
...
func (s *fetcherService) GetContent(detailURL string, chapterURL string, key string) (content novel.BookContent) {
	source, ok := sourceService.GetSourceByKey(key)
	if !ok {
		return
	}
...
func (s *fetcherService) parseClassifyInfo(source *novel.BookSource, doc *colly.XMLElement) (item *novel.BookInfo) {
	var (
...
func (s *fetcherService) parseItemSearch(source *novel.BookSource, doc *colly.XMLElement) (item *novel.BookInfo) {
...
func (s *fetcherService) parseItemInfo(source *novel.BookSource, doc *colly.XMLElement) (info novel.BookInfo) {
	var ele = fetcher.NewXMLElement(doc)
	info = novel.BookInfo{
...
func (s *fetcherService) parseChapterList(source *novel.BookSource, doc *colly.XMLElement, url string) (chapter novel.Chapter) {
	var ele = fetcher.NewXMLElement(doc)
...
func (s *fetcherService) parseContent(source *novel.BookSource, doc *colly.XMLElement, url string) (content novel.BookContent) {
	var ele = fetcher.NewXMLElement(doc)
	content = novel.BookContent{
